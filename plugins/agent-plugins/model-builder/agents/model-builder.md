---
name: model-builder
description: Builds DCF, DDM, LBO, three-statement, and trading-comps models live in Excel from a ticker and assumption set. Use when you need a clean model from scratch — not for updating an existing coverage model (use earnings-reviewer for that).
...
2. **DDM** — multi-stage DPS forecast, GGM terminal value, CAPM cost of equity, sensitivity tables.
3. **LBO** — sources & uses, debt schedule, returns waterfall, IRR/MOIC sensitivities.
4. **Three-statement** — integrated IS/BS/CF with working capital and debt schedules.
...
2. **Build the model.** Invoke the matching skill (`dcf-model`, `ddm-model`, `lbo-model`, `3-statement-model`, `comps-analysis`). Blue/black/green color coding; no hardcodes in calc cells.
...
`dcf-model` · `ddm-model` · `lbo-model` · `3-statement-model` · `comps-analysis` · `audit-xls`
